...
# ─────────────────────────────────────────────────────────────────────────────
# Registry — can_do responseSchema must declare unroutable[] (spec §6.3)
# ─────────────────────────────────────────────────────────────────────────────

class TestRegistryUnroutableSchema:

    @staticmethod
    def _can_do_schema(name):
        reg = Registry(name)
        ...
        return reg.get("can_do").to_dict()["responseSchema"]["properties"]

    def test_can_do_schema_declares_unroutable(self):
        """0.4.0 added unroutable[] to contracts and to the linter, but the
        Registry kept emitting the 0.3.0 schema. Every decorator-built server
        therefore tripped R015 with no way for its author to fix it: the schema
        is generated by the library, not written by the server."""
        assert "unroutable" in self._can_do_schema("test_unroutable_declared")

    def test_unroutable_items_carry_the_required_fields(self):
        props = self._can_do_schema("test_unroutable_fields")
        item_props = props["unroutable"]["items"]["properties"]
        assert {"outcome", "reason", "detail"} <= set(item_props)

    def test_unroutable_reason_is_constrained_to_the_spec_enum(self):
        props = self._can_do_schema("test_unroutable_enum")
        reason = props["unroutable"]["items"]["properties"]["reason"]
        assert set(reason["enum"]) == {
            "no_producer", "requirements_unmet", "malformed_request",
        }

    def test_unroutable_entry_shape_passes_R014(self):
        """R014 could never fire before: it only runs when `unroutable` is in
        props, and it never was."""
        from promcp.linter import lint_registry

        reg = Registry("test_unroutable_lint")

        @can_do_tool(description="Check feasibility.", registry=reg)
        def can_do(intent: dict) -> dict:
            return {"candidates": [], "blocked": []}

        results, _server = lint_registry(reg)
        codes = [f.code for r in results for f in r.findings]
        assert "R014" not in codes
        assert "R015" not in codes
